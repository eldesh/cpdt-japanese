...
%\index{Vernacular commands!Qed}%[Qed] コマンドは証明が実際に完了していることを確かめ、そうであればその証明を保存します。これまで書いてきたタクティクたちは[証明スクリプト]、別の言葉で言えば Ltac プログラムの列です。一方 [Qed] は正しく型付けされた Gallina の項である、証明項を生成するスクリプトの結果を使います。定理が正しいことは、証明スクリプト自体ではなく、証明項が正しいことの(比較的単純な)検査器のみで信用できます。本書の第1部では証明を Gallina の項として表現することの原理について紹介します。
...
一般に、HaskellやMLの多相型の具体化に似た場合でしか機能しません。
...
ソース言語の式とその型に加えて、初めのスタック型とそれに適合するスタックを量化します。
...
このコードは普段使うOCamlのコンパイラでコンパイルでき、そこそこ良いパフォーマンスの実行可能プログラムが得られます。
...
第1部の早い段階から強い自動証明のスタイルを取るので、経験豊富なCoqハッカーにも第1部は興味深いかもしれません。
*)
